...
class EntropyModel(nn.Module):
    ...
    def _create_local_causal_mask(self, seq_len):
        ...
        mask = (~allowed).float().masked_fill(~allowed, float("-inf")).masked_fill(allowed, 0.0)
        return mask
    ...
    def forward(self, x):
        ...
        if hasattr(self.config, "attn_bias_type") and self.config.attn_bias_type == "local_block_causal":
            if not hasattr(self.config, "sliding_window") or self.config.sliding_window is None:
                raise ValueError("sliding_window must be set when using attn_bias_type 'local_block_causal'")
